...
# mc1 <- init_meta() # will suffice, or you can specify parameters
mc1 <- init_meta(env_mean_mc = 0, env_sd_mc = 1, cauchy = 1, trait_sds = 0.5)
# Create the local habitat
# lh1 <- init_envt(npatch = 100) # will suffice, or
lh1 = init_envt(env_mean_lh = 2, env_sd_lh = 0.1, npatch = 100, gradient = "random")
# Draw the initial local community
lc1 = draw_lcom(mc = mc1, lh = lh1, nind = 10000)
```
### 2. Run simulation for the desired number of time steps (`nsteps` argument)
...
runsim <- run_sim(mc = mc1, lh = lh1, lc = lc1, 
  ...
  res_input = 10, 
  expenditure = 0.2, intake = 0.5, reproduction = 2)
# view the results over time
plot(runsim)
```
<img src="README/fig-0.png" width="672" style="display: block; margin: auto;" />

The diagnostic plot for simulation results might need some explanation:

- top left section depicts how trait structure of the local community changes over time; 
trait density distribution per time step codes the color of pixels, while the black line indicates the mean trait value of the entire community per time step.
The red dashed line corresponds to the mean trait value within the regional pool, while the blue dotted line corresponds to the mean trait value favored by the environmental conditions within the local habitat.
If environmental filtering takes place, one would expect the trait distribution to shift from the red line to the blue line over time.

- top right section depicts how average number of individuals per patch, mean individual biomass, amount of resource available per patch, and amount of resource consumed by the community changes over time.

- the bottom section looks at the local community at the last time step of the simulation, which should represent the community at its equilibrium state provided that the simulation had sufficient duration:

  - the bottom left section shows trait abundance distribution within the local community, with different layers corresponding to different species.
  Dotted line shows the expected trait distribution in a community of the same size randomly sampled from the regional pool.
  
  - the bottom right section shows the species abundance distribution of the resulted community (black line). 
  Gray dots correspond to a community of the similar size sampled randomly from the regional pool. 
  Discrepancies between these SADs indicate non-random nature of the local community.
...
**Initialize** a new metacommunity object with [`init_meta()`](R/4_init_meta.R).

Formal ***constructor & validator***: [`FilterABM_mc()`](R/1_FilterABM_mc.R).
...
**Initialize** a new local habitat object with [`init_envt()`](R/5_init_envt.R).

Formal ***constructor & validator***: [`FilterABM_lh()`](R/2_FilterABM_lh.R).
...
Formal ***constructor & validator***: [`FilterABM_lc()`](R/3_FilterABM_lc.R).
...
- `dem(lc = lc, mc = mc)` ensures that all individuals expend their biomass to survive a time step, eliminates all starving individuals, and simulates reproduction of all individuals whose biomass exceeds a threshold level;
