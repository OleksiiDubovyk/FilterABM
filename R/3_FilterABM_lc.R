...
#' \code{species} - integer; species ID,
#' \code{trait} - numeric; individual trait value,
#' \code{mass} - numeric; the current individual body mass,
#' \code{patch} - integer; the patch ID in which individual currently resides.
#'
...
validate_FilterABM_lc <- function(x){
  ...
  if (!all(c("species", "trait", "mass", "patch") %in% colnames(x))){
    stop(
      paste0("Column names in `FilterABM_lc` tibble must be: `species`, `trait`, `mass`, `patch`. Instead, ",
...
#'
#' Create a local community (\code{"FilterABM_lc"}) object.
#'
#' The \code{"FilterABM_lc"} class is based on \code{dplyr}'s tibbles, thus \code{dplyr} functions are applicable.
#'
#' The local community object must have the following columns:
...
#' (integer \code{species}, double \code{trait}, double \code{mass}, integer \code{patch}).
...
FilterABM_lc <- function(x, val_in = TRUE, val_out = TRUE){
  ...
  y <- y %>% select(species, trait, mass, patch)
  ...
  # trait density vs species
    ...
    ggplot2::geom_point(
      data = x %>%
        ...
        filter(n() == 1) %>%
        ungroup(),
      mapping = aes(x = trait, y = -1, color = factor(species)),
      show.legend = F
    ) +
